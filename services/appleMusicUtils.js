...
const getPlaylistData = async () => {
  return await axiosApple
    .get('/v1/me/library/playlist-folders/p.playlistsroot/children')
    .then((response) => {
...
const addMusicData = async (playlists) => {
  ...
  for (const [pid, playlist] of Object.entries(playlists)) {
    requests.push(
      axiosApple.get(
        playlist.url +
          '?include=tracks&include%5Blibrary-playlists%5D=catalog%2Ctracks'
      )
    );
  }
  const responses = await Promise.all(requests);
  responses.forEach((response) => {
    const pid = response.data.data[0].id;
    const musicAttributes = response.data.data[0].relationships.tracks.data;
    playlists[pid]['songData'] = musicAttributes.map((songAttribute) => {
      songAttribute = songAttribute['attributes'];
      return {
        artistName: songAttribute['artistName'],
        songName: songAttribute['name'],
        albumName: songAttribute['albumName'],
      };
    });
  });
  return playlists;
